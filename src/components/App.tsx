...
const App = ({ onYoutube, url }: ComponentProps) => {
  ...
  useEffect(() => {
    ...
    chrome.runtime.sendMessage({ queries }, (response) => {
      if (response === 'err') {
        setMessage(
          'Error: Reddit might be down or another extension is blocking Upvote. <a class="submit" target="_blank" href="https://github.com/z0ccc/Upvote-Anywhere#troubleshoot">Read more</a>'
        );

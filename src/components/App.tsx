...
const App = ({ onYoutube, sortSetting, url }: ComponentProps) => {
  ...
  useEffect(() => {
    ...
    chrome.runtime.sendMessage({ queries }, (response) => {
      if (response === 'err') {
        setMessage(
          'Error: Reddit might be down or another extension is blocking Voat. <a class="submit" target="_blank" href="https://github.com/z0ccc/voat-extension#troubleshoot">Read more</a>'
        );
